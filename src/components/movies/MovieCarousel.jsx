...
const MovieCarousel = ({ slides }) => {
  ...
  useEffect(() => {
    const width = window.innerWidth;
    if (width >= 1280) setSlideCount(6);
    else if (width >= 768) setSlideCount(3);
  ...
  return (
    <div className="flex justify-center relative transition-transform ease-out duration-700">
      <Button
        onClick={prevSlide}
        ...
        slides.slice(currentSlide, currentSlide + 2).map((s, i) => {
          return (
            <Card
              radius="none"
              key={i}
              className="col-span-12  sm:col-span-4 h-[300px] backdrop-blur-xl"
            >
              <Image
                removeWrapper
                alt="Card background"
                className={
                  "z-0 w-full brightness-75 h-full object-cover rounded-none"
                }
                src={s.posterUrl}
              />
